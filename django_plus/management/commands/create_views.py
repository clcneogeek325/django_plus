	...
	def handle(self, modelo=None, **options):
		if modelo is not None:
			...
			if encontrado:
				...
				print url_apps
				modelo_nombre_1 = getattr(apps, '%s'%modelo)
				models_1 = getattr(modelo_nombre_1, 'models')
				modelo = getattr(models_1, '%s'%modelo)
				lista_campos = modelo._meta.get_all_field_names()
				lista_campos.reverse()
